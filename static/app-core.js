(() => {
  ...
  function randomJapaneseStart(){ const list=MISSION_JA.filter(ch=>(JA_WORDS_BY_FIRST.get(ch)||[]).some(word=>wordAllowed(word)));return list.length?list[Math.floor(Math.random()*list.length)]:'あ'; }
